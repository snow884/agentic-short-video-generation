...
def main(weekend_id=0, town_id=0):
    ...
    chat_ollama_with_structured_output(
        user_prompt_params={"town_name": t.name, "state": t.state, "weekend_date": w.date, "event_list":json.dumps([asdict(row) for row in events]), "media_list": json.dumps([asdict(m) for m in media])  },
